...
import { deleteCity } from '../../core/_requests'
...
const ActionsCell: FC<Props> = ({id}) => {
  ...
  const deleteItem = useMutation(() => deleteCity(id), {
    // 💡 response of the mutation is passed to onSuccess
    onSuccess: () => {
      // ✅ update detail view directly
      queryClient.invalidateQueries([`${QUERIES.CITY_LIST}-${query}`])
    },
  })
  ...
  const deleteItemData= ()=>{
    MySwal.fire({
      title: 'Are you sure?',
      text: "You won't to be delete City?",
      icon: 'warning',
